describe('jasmineUnderTest.util', function() {
  ...
  describe('errorWithStack', function() {
    it('returns an Error with a stack', function() {
      var error = jasmineUnderTest.util.errorWithStack();

      expect(error).toEqual(jasmine.any(Error));
      expect(error.stack).toBeTruthy();
    });

    it('returns an Error with a stack from an N/error when necessary', function() {
      var originalError = Error;
      var errorWithoutStack = new Error('message');

      // Firefox was stubborn with deleting Non-configurable properties
      errorWithoutStack.stack = undefined;
      spyOn(jasmine.getGlobal(), 'Error').and.returnValue(errorWithoutStack);
      var nErrorSpy = jasmine.createSpyObj('N/error', {
        create: {
          stack: ['some', 'fake', 'stack']
        }
      });

      var util = getJasmineRequireObj().util(jasmineUnderTest, nErrorSpy);
      var error = util.errorWithStack();

      // Error has been spyed, reference the original
      expect(error).toEqual(jasmine.any(originalError));
      expect(nErrorSpy.create).toHaveBeenCalled();
      expect(error.stack).toBe('some\nfake\nstack');
    });
  });
});
